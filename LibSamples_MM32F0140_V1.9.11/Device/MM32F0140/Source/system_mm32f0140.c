...
///         pll_pdiv :PLL Divide Factor.
///         pll_mul : PLL Multiplication Factor.
///         pll_div : PLL Pre-divide Factor.
/// @retval amount of error
u32 AutoCalPllFactor(u32 pllclkSourceFrq, u32 pllclkFrq, u32* pll_pdiv, u32* pll_mul, u32* pll_div)
{
    u32 pdiv_temp, mul_temp, div_temp, pdiv_max, mul_max, div_max;
    u32 tempFrq, tempFvco;
    u32 minDiff = pllclkFrq;
    u8  flag = 0;
    
    div_max  = RCC_PLLCFGR_PLL_DP >> RCC_PLLCFGR_PLL_DP_Pos; //PLL Divide Factor.
    mul_max  = RCC_PLLCFGR_PLL_DN >> RCC_PLLCFGR_PLL_DN_Pos; //PLL Multiplication Factor.
    pdiv_max = RCC_PLLCFGR_PLL_DM >> RCC_PLLCFGR_PLL_DM_Pos; //PLL Pre-divide Factor.
    ...
    for (div_temp= 1; div_temp <= div_max; div_temp += 2)
    {
        for (pdiv_temp = 0; pdiv_temp <= pdiv_max; pdiv_temp++)
        {
            for (mul_temp = 0; mul_temp <= mul_max; mul_temp++)
            {
                ...
                if ((tempFvco > 200000000) ||(tempFvco < 80000000))
                {
                    continue;
                }
...
static void SetSysClockToXX_HSE(void)
{
    __IO u32 temp, tn, tm, tp;
    __IO u32 StartUpCounter = 0, HSEStatus = 0;

    u32 plln, pllm, pll_pdiv;
    ...
    AutoCalPllFactor(HSE_VALUE, SystemCoreClock, &pll_pdiv, &plln, &pllm);

    RCC->PLLCFGR &= ~((u32 ) RCC_PLLCFGR_PLLSRC | RCC_PLLCFGR_PLLXTPRE);
    RCC->PLLCFGR |= (u32 ) RCC_PLLCFGR_PLLSRC  ;

    RCC->APB1ENR |= RCC_APB1ENR_PWR;
    tm = (((u32)pllm << RCC_PLLCFGR_PLL_DP_Pos) & RCC_PLLCFGR_PLL_DP); //PLL Divide Factor.
    tn = (((u32)plln << RCC_PLLCFGR_PLL_DN_Pos) & RCC_PLLCFGR_PLL_DN); //PLL Multiplication Factor.
    tp = (((u32)pll_pdiv << RCC_PLLCFGR_PLL_DM_Pos) & RCC_PLLCFGR_PLL_DM); //PLL Pre-divide Factor.
    RCC->PLLCFGR &= (u32)((~RCC_PLLCFGR_PLL_DN) & (~RCC_PLLCFGR_PLL_DP) & (~RCC_PLLCFGR_PLL_DM));
    RCC->PLLCFGR |= tn | tm | tp;
...
static void SetSysClockToXX_HSI(void)
{
    ...
    AutoCalPllFactor(HSI_VALUE_PLL_ON, SystemCoreClock, &pll_pdiv, &plln, &pllm);
